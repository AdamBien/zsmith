...
/// Traces agent spec R5.2 — see src/main/java/airhacks/zsmith/agent/package-info.java
...
void main() throws Exception {
    ...
    try {
        runtimeSessionsGetIsolatedMemory();
    ...
    // withAgentCoreServer keeps non-daemon server threads alive by design (long-running runtime
    // containers), so the IT must exit explicitly or the JVM never terminates
    System.exit(0);
}
...
// R5.2 — When serving the AgentCore runtime contract, the BC shall give each runtime session an
// isolated agent clone with its own conversation memory and shared tools and configuration.
void runtimeSessionsGetIsolatedMemory() throws Exception {
    int port;
    try (var probe = new ServerSocket(0)) {
        port = probe.getLocalPort();
    }
    new Agent("agentcore-session-agent", "prompt").withAgentCoreServer(port);

    try (var client = HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(2)).build()) {
        invoke(client, port, "alpha", "a1");
        invoke(client, port, "alpha", "a2");
        invoke(client, port, "beta", "b1");
    }

    var alphaFollowUp = requestWithLastMessage("a2");
    var contents = messageContents(alphaFollowUp);
    if (!contents.contains("a1"))
        throw new AssertionError("R5.2 — expected runtime session alpha to keep its own history but got: " + contents);
    if (contents.contains("b1"))
        throw new AssertionError("R5.2 — expected runtime session alpha isolated from beta but got: " + contents);

    var betaFirst = requestWithLastMessage("b1");
    if (betaFirst.getJSONArray("messages").length() != 1)
        throw new AssertionError("R5.2 — expected a fresh memory for runtime session beta but got: "
                + betaFirst.getJSONArray("messages"));
}

void invoke(HttpClient client, int port, String sessionId, String prompt) throws Exception {
    var payload = new JSONObject().put("prompt", prompt).toString();
    var request = HttpRequest.newBuilder(URI.create("http://localhost:" + port + "/invocations"))
            .timeout(Duration.ofSeconds(10))
            .header(RUNTIME_HEADER, sessionId)
            .POST(BodyPublishers.ofString(payload))
            .build();
    var response = client.send(request, BodyHandlers.ofString());
    if (response.statusCode() != 200)
        throw new AssertionError("R5.2 — test setup expected 200 from /invocations but got: " + response.statusCode());
}
...
JSONObject requestWithLastMessage(String content) {
    return this.llmRequests.stream()
            ...
            .findFirst()
            .orElseThrow(() -> new AssertionError("R5.2 — no LLM request ends with message: " + content));
}
